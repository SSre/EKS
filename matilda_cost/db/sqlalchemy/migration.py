...
from matilda_cost.db.sqlalchemy import api as db_session

INIT_VERSION = {}
INIT_VERSION['matilda_cost'] = 0
_REPOSITORY = {}

LOG = logging.getLogger(__name__)


def get_engine(database='matilda_cost', context=None):
    if database == 'matilda_cost':
        return db_session.get_engine()


def db_sync(version=None, database='matilda_cost', context=None):
...
def db_version(database='matilda_cost', context=None):
    repository = _find_migrate_repo(database)
    try:
...
def db_version_control(version=None, database='matilda_cost', context=None):
    repository = _find_migrate_repo(database)
...
def _find_migrate_repo(database='matilda_cost'):
